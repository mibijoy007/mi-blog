...
export function getAllBlogLinks() {
  ...
  // ✅ Always derive paths from process.cwd() so OpenNext resolves bundled content properly
...
links.push(relativePath.startsWith('/') ? relativePath : '/' + relativePath);
      }
    }
  }
...
export default async function getAllBlogsData() {
  ...
  for (const link of links) {
    // ✅ Construct clean absolute path from process.cwd()
    const finalFileLink = path.join(process.cwd(), 'content', link, 'page.mdx');
   
    try {
      if (!fs.existsSync(finalFileLink)) {
        continue;
      }
      ...
      if (compiledMDX.frontmatter && link) {
        ...
        if (frontmatterDate) {
          ...
          try {
            const fileStats = fs.statSync(finalFileLink);
            date = fileStats.birthtime?.toISOString() || new Date().toISOString();
          } catch {
        ...
        const tags = Array.isArray(compiledMDX.frontmatter.tags)
